...
class TripwireSql:
    ...
    def _get_comments(self, system_id):
        query = "SELECT id, comment FROM comments WHERE systemID=%s AND createdById=%s AND maskID=%s"
        self.cursor.execute(query, (system_id, self.trip_char_id, self.mask))
        return self.cursor.fetchall()

    def _add_comment(self, system_id, comment_text):
        query = """\
            INSERT INTO comments (systemID, comment, created, createdById, createdByName, modifiedById, modifiedByName, maskID)
            VALUES (%s, %s, %s, %s, "BountyBot", %s, "BountyBot", %s)\
            """
        self.cursor.execute(
    ...
    def _edit_comment(self, comment_id, comment_text):
        query = "UPDATE comments SET comment=%s, modified=%s, modifiedById=%s, modifiedByName=%s WHERE id=%s"
        self.cursor.execute(query, (comment_text, self._time_now(), self.trip_char_id, "BountyBot", comment_id))
